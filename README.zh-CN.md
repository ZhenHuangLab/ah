...
要登录，在这台机器上运行 `ah login`。它会打印一个 10 分钟内有效的链接，并附上方便手机扫描的二维码；在浏览器里打开链接并确认后，会保持登录 30 天。在 tailnet 里的浏览器，或者已经登录的浏览器，可以在命令列表里选择 “Sign in on another device”，为另一台设备显示这样的链接。
...
分享链接开放对应的快照，登录链接授予全部会话的访问权限。登录链接在 10 分钟内可重复使用，已登录设备可以生成新链接。要撤销所有登录，删除 `~/.local/share/ah/key` 并重启 `ah serve`。停止分享链接不会删除读者已保存的副本。
